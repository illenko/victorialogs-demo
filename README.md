# Spring Boot + VictoriaMetrics Observability Demo

A demo project showcasing how to use the full [VictoriaMetrics](https://victoriametrics.com/) stack — **VictoriaMetrics**, **VictoriaLogs**, and **VictoriaTraces** — with a Spring Boot application, leveraging their OpenTelemetry-compatible
receivers for metrics, log, and trace ingestion.
...
The application starts and immediately begins generating synthetic API traffic (~5 requests/second).
...
Spring Boot auto-instruments HTTP server requests via Micrometer and exports them using the OTLP protocol directly to
VictoriaMetrics:
...
The `logback-spring.xml` uses a split routing trick: the `interactions` logger is attached to the OTEL appender
(so structured request/response logs are exported to VictoriaLogs) but **not** to the console — keeping terminal output
clean. The root logger only writes to console. This means `RequestLoggingFilter` interaction logs flow exclusively to
VictoriaLogs, while business logs from controllers appear in both console and VictoriaLogs.

Logs are structured as key-value pairs (requestId, tenantId, method, URI, status, latency, request/response bodies)
making them queryable in VictoriaLogs.

### Traces pipeline

Spring Boot auto-instruments HTTP server and client requests with OpenTelemetry and exports traces directly to
VictoriaTraces:
...
The scheduled traffic generator makes real HTTP calls via `RestClient`, which automatically creates client→server trace
spans. The payment endpoints also create custom child spans (`payment.lookup`, `payment.enrich`, `payment.validate`,
`payment.process`) to produce multi-span traces.

### No collector needed

VictoriaMetrics, VictoriaLogs, and VictoriaTraces all natively support OpenTelemetry OTLP HTTP receivers, so the
application exports telemetry **directly** — no OpenTelemetry Collector is required.
...
A scheduled task calls these endpoints every 200ms with random tenant IDs and request parameters, simulating realistic
multi-tenant traffic with varying response codes and latencies.
...
- **VictoriaTraces v0.7.1** — distributed tracing backend (Jaeger-compatible)
